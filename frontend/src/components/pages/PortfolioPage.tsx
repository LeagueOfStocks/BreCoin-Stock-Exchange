...
// Singleton to track if PortfolioPage has been rendered before
let hasPortfolioRenderedBefore = false;
let cachedHoldings: Holding[] = [];
let cachedTransactions: Transaction[] = [];
let cachedGold: number = 0;
...
export default function PortfolioPage() {
  ...
  const [gold, setGold] = useState<number>(cachedGold)
  const [loading, setLoading] = useState(false) // Start with false
  const [initialized, setInitialized] = useState(hasPortfolioRenderedBefore) // Track initialization

  useEffect(() => {
    if (!user) {
        setLoading(false);
        setInitialized(true);
        hasPortfolioRenderedBefore = true;
        return;
    }

    const fetchData = async () => {
      // Only show loading state on initial load when we have no data
      if (!initialized && !hasPortfolioRenderedBefore) {
        setLoading(true);
      }
      ...
      if (holdingsData) {
        ...
        cachedHoldings = holdingsData; // Cache globally
      }
      if (holdingsError) {
      ...
      if (transactionsData) {
        ...
        cachedTransactions = transactionsData as Transaction[]; // Cache globally
      }
    ...
    fetchData();
  }, [user?.id]); // Remove 'initialized' from dependencies

  // Only show loading skeleton on true initial load
  if (loading && !initialized && !hasPortfolioRenderedBefore) {
    return (
