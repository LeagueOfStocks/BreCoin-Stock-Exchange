...
let hasTopRenderedBefore = false;
let cachedTopPerformers: { top_performers: Performer[], bottom_performers: Performer[] } = { top_performers: [], bottom_performers: [] };
...
export default function TopPerformersPage() {
    ...
    const [performers, setPerformers] = useState<{ top_performers: Performer[], bottom_performers: Performer[] }>(cachedTopPerformers);
    const [timeframe, setTimeframe] = useState('1m'); // Corresponds to 'month'
    const [loading, setLoading] = useState(false);
    const [initialized, setInitialized] = useState(hasTopRenderedBefore);
    ...
    useEffect(() => {
        ...
        if (!currentMarket) {
            ...
            hasTopRenderedBefore = true;
            return;
        }

        const fetchPerformers = async () => {
            // Only show loading on first initialization
            if (!initialized && !hasTopRenderedBefore) {
                setLoading(true);
            }
            ...
            try {
                ...
                if (data && data.top_performers) {
                    setPerformers(data);
                    cachedTopPerformers = data;
                } else {
                    const emptyData = { top_performers: [], bottom_performers: [] };
                    setPerformers(emptyData);
                    cachedTopPerformers = emptyData;
                }

            } catch (error) {
                console.error('Error fetching performers:', error);
                if (!hasTopRenderedBefore) {
                    setPerformers({ top_performers: [], bottom_performers: [] });
                }
            } finally {
                setLoading(false);
        ...
        fetchPerformers();
    }, [currentMarket?.id, timeframe, marketInitialized]); // Remove 'initialized' from dependencies

    // Only show loading skeleton on initial load when market context is not ready
    if (!marketInitialized || (loading && !initialized && !hasTopRenderedBefore)) {
        return (
