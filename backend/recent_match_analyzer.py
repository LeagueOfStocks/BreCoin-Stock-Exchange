        ...
        try:
            ...
            formatted_data = {
                ...
                "performance": {
                    ...
                    "healing": player_data.get('totalHealsOnTeammates', 0),
                    "shielding": player_data.get('totalDamageShieldedOnTeammates')
                },
                "objectives": {
...
if __name__ == "__main__":
    ...
    # Get PUUID
    game_name, tag_line = SUMMONER_NAME.split('#')
    account_data = analyzer.get_account_by_riot_id(game_name, tag_line)
    puuid = account_data['puuid']

    # Get latest match ID
    latest_match_id = analyzer.get_latest_match_id(puuid)
    if not latest_match_id:
        print("No matches found for this player.")
    else:
        result = analyzer.get_match_data(latest_match_id, SUMMONER_NAME)
